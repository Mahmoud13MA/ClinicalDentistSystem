...
namespace clinical.APIs.Modules.Radiology.Controllers
{
    ...
    public class RadiologistAuthController(AppDbContext context, IJwtService jwtService, IPasswordHashService passwordHashService, IConfiguration configuration, IEmailValidationService emailValidationService) : ControllerBase
    {
        [HttpPost("Register")]
        public async Task<IActionResult> Register([FromBody] RadiologistRegisterRequest request)
        {
            ...
            return Ok(new RadiologistLoginResponse
            {
                Token = token,
